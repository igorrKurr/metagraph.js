  ...
  function (AbstractGraph, Renderer, MetaElement, Config, Edge) {
    ...
    var Metagraph = function () {
      ...
      proto.prepareLevel = function(x, y, level){
        [].forEach.call(level, function(el, index, array){
          el.prepareWithChildren(x, y);
          if (array[_index_minus_one(index)].has('children')) {
            x = array[_index_minus_one(index)].lastChildrenCoordinates.x + 2*Config.radius+Config.distance;
          }
          else {
      ...
      var _index_minus_one = function(index) {
        if (index === 0 ) { return 0;}
        return index - 1;
      }
      proto.levels = function (type) {
        ...
        var vertices = this.vertices.map(function(el){
          el.isChecked = false;
          return el;
        });
        return AbstractGraph.levels(vertices, relationType);
      };
